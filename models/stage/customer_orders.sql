...
from {{ source ('shop-data', 'orders')}} orders

join (
      select 
        first_name || ' ' || last_name as name, 
        * 
      from {{ source ('shop-data', 'customers')}}
) customers
...
join (
    ...
    left outer join {{ source ('shop-data', 'payments')}} c
    on a.id = c.orderid
...
left outer join {{ source ('shop-data', 'payments')}} payments
